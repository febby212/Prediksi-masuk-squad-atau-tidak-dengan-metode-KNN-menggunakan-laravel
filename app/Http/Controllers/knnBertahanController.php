...
class knnBertahanController extends Controller
{
    function index()
    {
        if (Auth::check()) {
            return view('user.formBek', [
                "title" => "Prediksi Bertahan"
            ]);
        }
    ...
    public function predict(Request $request)
    {
        ...
        $players = Bertahan::all();
        $nilai_k = nilaiK::where('id', 3)->value('nilai_k');
        ...
        $nearestPlayers = Bertahan::whereIn('id', $nearestPlayerIds)->get();
    ...
    public function predictAdmin(Request $request)
    {
        // Ambil data test dari input form
        $testData = [
            'nama' => $request->input('nama'),
        ...
        // Tampilkan hasil prediksi dan pemain terdekat pada view
        return view('user.hasilAdmin', ["title" => "Hasil Prediksi"], [
            'testData' => $testData,
            'nearestPlayers' => $nearestPlayers,
            'prediction' => $prediction,
            'percentage' => $percentage,
